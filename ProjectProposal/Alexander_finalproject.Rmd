...
> Was there a significant difference in the proportion of GOP votes by county in the 2016 presidential election based on county type (Rural, Suburban, or Urban)?
...
Here are the libraries I used for my project:  
...
I then loaded the data from my GitHub account, after downloading the files from the links provided below:  
...
After this was set up, I could then look at summary statistics of the `per_gop` variable:  
```{r, echo=FALSE}
...
To run some tests on our 3-way ruralness variable (`ruralurban_grp_3_way`), we'll be using Analysis of Variance (ANOVA). There are three conditions that need to be checked before running an ANOVA.

* All observations must be independent.
    + The election and ruralness data were not sampled, however, there is not an obvious reason why independence of observations would not hold for most or all of the values in the dataset.  **This is satisfied.**
* The data in each group must be nearly normal.
    + Given the large sample size for each group, we should be fine, but in order to be safe I removed a some outliers from the full dataset in order to ensure that each group was approaching a normal distribution. In total, I removed 189 counties from the dataset. **This is satisfied.**
...
The summary from our regression model in the previous section also contains values to create a linear regression equation that we can use to predict the percent of votes for Trump in the 2016 election given then percentage of individuals with a college degree or more for a given county:  
...
```{r, echo=FALSE, fig.height=10, fig.width=10, fig.align="center", warning=FALSE, message=FALSE}
library(sf)
library(maps)
